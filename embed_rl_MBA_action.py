...
def main():
    ...
    with h5py.File(args.filename, 'w') as f_out, \
        ...
        if not args.quiet:
            print('Restoring from checkpoint: {} and {}'.format(checkpoint_sup, checkpoint_rl))
        ...
        for start_idx in count(step=args.batch_size):
            try:
                b_atts = sess_sup.run(atts)
                b_atts = np.transpose(np.mean(b_atts, axis=(2, 3)), (1, 0, 2))
                action = Agent.choose_action(np.sum(b_atts, axis=1))
                ...
                slt_ftrs = np.sum(b_atts * np.expand_dims(action, axis=2), axis=1)
                if args.rl_residual:
                    slt_ftrs += np.mean(b_atts, axis=1)
                emb = sess_sup.run(endpoints['emb'], feed_dict={endpoints['model_output']:slt_ftrs})
                # emb = sess_sup.run(endpoints['emb'], feed_dict={endpoints['model_output']:np.sum(b_atts * np.expand_dims(action, axis=2), axis=1)})
