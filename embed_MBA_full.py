...
parser.add_argument(
    '--rl_residual', action='store_true', default=False,
    help='use residual connection')
parser.add_argument(
    '--emb_feature', action='store_true', default=False)
...
def main():
    ...
    with h5py.File(args.filename, 'w') as f_out, \
        ...
        if args.checkpoint is None:
            checkpoint_sup = tf.train.latest_checkpoint(args.experiment_root, latest_filename='checkpoint-sup')
        else:
            checkpoint_sup = os.path.join(args.experiment_root, '{}-sup-{}'.format(*(args.checkpoint.split('-'))))
        ...
        emb_storage = np.zeros(
            (7, len(data_fids) * len(modifiers), args.embedding_dim), np.float32)
        if args.emb_feature:
            ftr_emb_storage = np.zeros(
            (len(data_fids) * len(modifiers), 1536), np.float32)
        for start_idx in count(step=args.batch_size):
            try:
                # b_atts, b_embs, b_ftrs = sess_sup.run([atts, endpoints['emb'], endpoints['model_output']])
                b_atts, b_embs, b_ftrs = sess_sup.run([atts, endpoints['emb'], endpoints['Mixed_7d']])
                b_atts = np.mean(b_atts, axis=(2, 3))
                for b_idx in range(5):
                    emb = sess_sup.run(endpoints['emb'], feed_dict={endpoints['model_output']:b_atts[b_idx]})
                    emb_storage[b_idx, start_idx:start_idx + len(emb)] = emb
                b_ftrs = np.mean(b_ftrs, (1, 2))
                emb_storage[5, start_idx:start_idx + len(b_embs)] = b_embs
                emb = sess_sup.run(endpoints['emb'], feed_dict={endpoints['model_output']:b_ftrs})
                emb_storage[6, start_idx:start_idx + len(emb)] = emb
                if args.emb_feature:
                    ftr_emb_storage[start_idx:start_idx + len(b_ftrs)] = b_ftrs
                print('\rEmbedded batch {}-{}/{}'.format(
                    start_idx, start_idx + len(b_embs), len(emb_storage[5])), 
                    flush=True, end='')
        ...
        if len(modifiers) > 1:
            ...
            emb_storage = emb_storage.reshape(len(data_fids), len(modifiers), -1).transpose((1,0,2))
        ...
        for b_idx in range(5):
            ...
            emb_dataset = f_out.create_dataset('emb_b{}'.format(b_idx), data=emb_storage[b_idx])
        f_out.create_dataset('emb_full', data=emb_storage[5])
        f_out.create_dataset('emb_raw', data=emb_storage[6])
        if args.emb_feature:
            ftr_emb_dataset = f_out.create_dataset('ftr', data=ftr_emb_storage)
