...
def sample_k_fids_for_model(model, all_fids, all_pids, all_models, all_colors, batch_k, batch_p):
    """ Given a PID, select K FIDs of that specific PID. """
    # possible_fids = tf.boolean_mask(all_fids, tf.equal(all_models, model))
    # possible_pids = tf.boolean_mask(all_pids, tf.equal(all_models, tf.cast(model, tf.int32)))
    possible_pids = tf.boolean_mask(all_pids, tf.equal(all_models, model))
    unique_pids, _ = tf.unique(possible_pids)

    # The following simply uses a subset of K of the possible FIDs
    # if more than, or exactly K are available. Otherwise, we first
    # create a padded list of indices which contain a multiple of the
    # original FID count such that all of them will be sampled equally likely.
    count = tf.shape(unique_pids)[0]
    padded_count = tf.cast(tf.ceil(batch_p / count), tf.int32) * count
    full_range = tf.mod(tf.range(padded_count), count)

    # Sampling is always performed by shuffling and taking the first k.
    shuffled = tf.random_shuffle(full_range)
    selected_pids = tf.gather(unique_pids, shuffled[:batch_p])

    selected_fids, selected_pids = tf.map_fn(lambda pid:sample_k_fids_for_pid(pid, 
        all_fids=all_fids, all_pids=all_pids, batch_k=batch_k), 
        selected_pids, dtype=(tf.string, tf.string))

    selected_fids = tf.reshape(selected_fids, [-1])
    selected_pids = tf.reshape(selected_pids, [-1])
    # selected_models = tf.fill([tf.shape(selected_fids)[0]], model)
    selected_models = tf.fill([batch_k * batch_p], model)
    # selected_models = tf.map_fn(lambda fid:tf.boolean_mask(all_models, tf.equal(all_fids, fid)), selected_fids)
    selected_colors = tf.map_fn(lambda fid:tf.boolean_mask(all_colors, tf.equal(all_fids, fid)), selected_fids)
    # print(selected_fids, selected_pids, selected_models, selected_colors)

    # return selected_fids, tf.fill([batch_k], pid)
    return selected_models, selected_colors, selected_fids, selected_pids
...
def main():
    ...
    '''
    train_block = {}
    for model_idx in range(len(fids)):
        if models[model_idx] not in train_block.keys():
            train_block[models[model_idx]] = {pids[model_idx]:fids[model_idx]}
        elif pids[model_idx] not in train_block[models[model_idx]].keys():
            train_block[models[model_idx]][pids[model_idx]] = [fids[model_idx]]
        else:
            train_block[models[model_idx]][pids[model_idx]].append(fids[model_idx])
    print(train_block)
    '''
    # unique_pids = np.unique(pids)
    unique_models = np.unique(models)
    dataset = tf.data.Dataset.from_tensor_slices(unique_models)
    dataset = dataset.shuffle(len(unique_models))
    ...
    dataset = dataset.map(lambda model: sample_k_fids_for_model(
        model, all_fids=fids, all_pids=pids, 
        all_models=models, all_colors=colors, 
        batch_k=args.batch_k, batch_p=args.batch_p))
    print('flatten {} {}'.format(dataset.output_shapes, dataset.output_types))
    ...
    dataset = dataset.map(
        lambda c_model, c_color, fid, pid: common.fid_to_image_cls(
            c_model, c_color, fid, pid, image_root=args.image_root,
    ...
    if args.flip_augment:
        dataset = dataset.map(
            lambda im, c_model, c_color, fid, pid: (tf.image.random_flip_left_right(im), c_model, c_color, fid, pid))
    ...
    images, c_models, c_colors, fids, pids = dataset.make_one_shot_iterator().get_next()
    ...
    tf.summary.histogram('embedding_lengths', tf.norm(endpoints['emb_raw'], axis=1))
    ...
    trainable_variables = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, '^(?!.*({}|{})).*$'.format(body_prefix, 'emb'))
    # trainable_variables = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, '^(?!{}).*$'.format(body_prefix))
    print('trainable variables {}'.format(trainable_variables))
    ...
    with tf.Session(config=config) as sess:
        ...
        with lb.Uninterrupt(sigs=[SIGINT, SIGTERM], verbose=True) as u:
            for i in range(start_step, args.train_iterations):
                ...
                _, summary, step, b_prec_at_k, b_embs, b_loss, b_fids, b_models = \
                    sess.run([train_op, merged_summary, global_step,
                              prec_at_k, endpoints['emb'], losses, fids, c_models])
                elapsed_time = time.time() - start_time
                # log.info('models in a batch {}'.format(b_models))
