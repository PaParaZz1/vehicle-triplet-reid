...
class NpairLoss(nn.Module):
    ...
    def __init__(self, gpu, l2_reg=0.002):
        super(NpairLoss, self).__init__()
        self.l2_reg = l2_reg
        self.gpu = gpu

    def _remap(self, feats, labels):

        def anchor_idxs(c):
            return labels_cpu.eq(c).nonzero()[0].squeeze(0)

        def positive_idxs(c):
            return labels_cpu.eq(c).nonzero()[1].squeeze(0)
        ...
        anchor_inds = torch.stack(list(map(anchor_idxs, classes)))
        # print(anchor_inds)
        positive_inds = torch.stack(list(map(positive_idxs, classes)))

        # print(anchor_inds)
        # print(positive_inds)

        anchors = feats_cpu[anchor_inds]
        positives = feats_cpu[positive_inds]

        return anchors, positives, classes

    def forward(self, x, y):
        anchor, positive, target = self._remap(x, y)
        anchor = anchor.cuda(self.gpu)
        positive = positive.cuda(self.gpu)
