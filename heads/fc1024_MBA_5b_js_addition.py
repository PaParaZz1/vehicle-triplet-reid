        ...
        with slim.arg_scope([slim.batch_norm], **batch_norm_params):
            ...
            for i in range(head_num):
                for j in range(i + 1, head_num):
                    ...
                    js_div = js_divergence(masks[i], masks[j], 'constraint_{}{}'.format(i, j))
...
def js_divergence(mask_a, mask_b, prefix):
    ...
    kl_div_am = tf.reduce_sum(dist_a * tf.log(dist_a / (dist_m + 1e-6)), name='{}_kl_div_am'.format(prefix), axis=1)
    kl_div_bm = tf.reduce_sum(dist_b * tf.log(dist_b / (dist_m + 1e-6)), name='{}_kl_div_bm'.format(prefix), axis=1)
    js_div = CONSTRAINT_WEIGHT * (tf.log(2.) - (kl_div_am + kl_div_bm) / 2)
