    ...
    with slim.arg_scope(
        ...
        with slim.arg_scope([slim.batch_norm], **batch_norm_params):
            ...
            for i in range(head_num):
                ...
                masked_map = attention_branch_mask * endpoints['resnet_v2_50/block4']
...
def cosine_similarity(mask_a, mask_b, prefix):
    ...
    cosine_sim = CONSTRAINT_WEIGHT * tf.reduce_sum(tf.multiply(dist_a, dist_b, name='{}_cosine_similarity'.format(prefix)), axis=1)
    # cosine_sim = CONSTRAINT_WEIGHT * tf.tensordot(dist_a, dist_b, axes=1, name='{}_cosine_similarity'.format(prefix))
