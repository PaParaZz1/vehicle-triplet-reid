...
def kl_divergence(mask_a, mask_b, prefix):
    ...
    kl_div_ab = tf.reduce_sum(dist_a * (tf.log(dist_a) - tf.log(dist_b)), axis=1, name='{}_kl_div_ab'.format(prefix))
    # kl_div_ba = tf.distributions.kl_divergence(dist_b, dist_a, name='{}_kl_div_ba'.format(prefix))
    # kl_div_ba = tf.reduce_sum(dist_b * (tf.log(dist_b) - tf.log(dist_a)), axis=1, name='{}_kl_div_ba'.format(prefix))
    # kl_div = - CONSTRAINT_WEIGHT * (kl_div_ab + kl_div_ba) / 2
    tf.losses.add_loss(kl_div_ab)
