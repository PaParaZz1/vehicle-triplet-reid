...
def joint_batch_hard(dists, dists_origin, pids, margin, batch_precision_at_k=None):
    """Computes the batch-hard loss from arxiv.org/abs/1703.07737.
    ...
    Returns:
        A 1D tensor of shape (B,) containing the loss value for each sample.
    """
    with tf.name_scope("joint_batch_hard"):
        same_identity_mask = tf.equal(tf.expand_dims(pids, axis=1),
                                      tf.expand_dims(pids, axis=0))
        negative_mask = tf.logical_not(same_identity_mask)
        positive_mask = tf.logical_xor(same_identity_mask,
        ...
        print('shape | dists {} | pos_mask {} | fp_idx {} | cn_idx {}'.format(tf.shape(dists), 
            tf.shape()))
        fp_idx = tf.argmax(dists_origin*tf.cast(positive_mask, tf.float32), axis=1)
        cn_idx = tf.map_fn(lambda x: tf.argmin(tf.boolean_mask(x[0], x[1])),
                    (dists_origin, negative_mask), dtype=tf.int64)
        # furthest_positive = tf.reduce_max(dists*tf.cast(positive_mask, tf.float32), axis=1)
        # closest_negative = tf.map_fn(lambda x: tf.reduce_min(tf.boolean_mask(x[0], x[1])),
        #                             (dists, negative_mask), tf.float32)
        # furthest_positive = tf.gather(dists, tf.cast(fp_idx, tf.int32), axis=1)
        furthest_positive = tf.gather(dists, fp_idx, axis=1)
        closest_negative = tf.gather(dists, cn_idx, axis=1)
        # closest_negative = tf.gather(dists, tf.cast(cn_idx, tf.int32), axis=1)
        # Another way of achieving the same, though more hacky:
        # closest_negative = tf.reduce_min(dists + 1e5*tf.cast(same_identity_mask, tf.float32), axis=1)
...
LOSS_CHOICES = {
    ...
    'joint_batch_hard': joint_batch_hard,
}
