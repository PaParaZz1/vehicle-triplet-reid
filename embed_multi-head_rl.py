...
import common
from RL_utils import PolicyGradient,PolicyGradient_MultiHead
...
def main():
    ...
    Agent = PolicyGradient_MultiHead(n_actions=5,
                        n_features=1536*5,
                        is_train=False,
    ...
    with h5py.File(args.filename, 'w') as f_out, \
        ...
        for start_idx in count(step=args.batch_size):
            try:
                ...
                repeated_aciton = np.repeat(action, 1536, axis=1)
                emb = sess_sup.run(endpoints['emb'], feed_dict={endpoints['model_output']:b_ftrs * repeated_aciton})
