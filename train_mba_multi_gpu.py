...
def main():
    ...
    loss_mean = make_parallel(get_model, 
                            ...
                            fids=fids, 
                            pids=pids)
    print('get loss {}'.format(tf.losses.get_losses()))

    mbd_loss = tf.add_n(tf.losses.get_losses())
    print('mbd loss {}'.format(mbd_loss))
    loss_mean = loss_mean + tf.reduce_mean(mbd_loss, keep_dims=True)
    ...
    with tf.Session(config=config) as sess:
        ...
        with lb.Uninterrupt(sigs=[SIGINT, SIGTERM], verbose=True) as u:
            for i in range(start_step, args.train_iterations):
                ...
                _, summary, step, b_prec_at_k, b_embs, b_loss, b_fids, mbd = \
                    sess.run([train_op, merged_summary, global_step,
                              prec_at_k, endpoints['emb'], losses, fids, mbd_loss])
