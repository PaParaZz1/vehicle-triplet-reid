...
parser.add_argument(
    '--branch_name', nargs='+', type=common.nonnegative_int,
    help='which branch to use for embedding')
        ...
        for start_idx in count(step=args.batch_size):
            try:
                if args.branch_name is None:
                    b_ftrs = sess.run(endpoints['Mixed_7d'])
                    b_ftrs = np.mean(b_ftrs, axis=(1, 2), keepdims=False)
                else:
                    slt_branch = np.zeros((5, 1, 1))
                    for branch_idx in args.branch_name:
                        slt_branch[branch_idx][0][0] = 1
                    b_ftrs = sess.run(atts)
                    # b_ftrs = b_ftrs * np.repeat(slt_branch, 1536, axis=1)
                    b_ftrs = np.mean(b_ftrs, axis=(2, 3), keepdims=False)
                    b_ftrs = np.sum(b_ftrs * slt_branch, axis=0)
                emb = sess.run(endpoints['emb'], feed_dict={endpoints['model_output']:b_ftrs})
